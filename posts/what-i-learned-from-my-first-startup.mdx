...
Right off the bat we were able to tell that this was a painful problem.
...
These are guys that flip shoes on the side or owners of small businesses that wanted to advertise their products better.
...
uses it, the entirety of our market is on it. The entirety of the sales cycle happens inside Telegram.
...
I remember having the epiphany of our MVP during class. It sounds corny but it really was one of those "flashing lightbulb" moments.
...
A seller would get a link from the Seller Bot to post their items on webapp that would then store the info on a database.
...
A lot of it was my fault though. I can't put all the blame on the developers for the product not being built. What we assumed was an MVP was not an MVP at all.
There were a host of features that weren't 'minimal' at all. An analytics dashboard for sellers, storefronts on the website,
and a bunch of other unnecessary shit that convoluted what was supposed to be a simple test to validate underlying assumptions. Instead,
we (me especially) were treating it like it was going to be the next iPhone. 
...
Market research on spreadsheets (useful but not a priority), figuring out the cap table (a big point of contention), and the worst one of all: writing an 8 page business plan
(a useful exercise in pretending to be an entrepreneur but accomplished nothing in the process).
...
putting it in the hands of users in my senior year of highschool but I stopped working on it after starting university. 
...
have a foundational understanding of how the technology works. Even if you have a technical cofounder, you dont want to be completely oblivious 
...
That woudln't have stopped Abay Shops from failing, but it would have definitely saved time and money.
...
One example that comes to mind is a duo of founders who've been working on their startup for well over 6 months. They have a web app,
a native app, and a custom server they spent three hours building.
...
It's not that these things have no value. But the improper application of them can be a colossal waste of time. 
...
But this too is a post for another time.
